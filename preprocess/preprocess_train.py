...
all_stories = load_json('./all_stories_train.json')
all_titles = load_json('./all_titles_train.json')


triples_dict = load_json('./triples_train.json')
...
def preprocess_all_stories():
    for i, t in enumerate(all_titles):
        ...
        file = open('./train_data/' + t + '.json', 'w')
        file.close()
        save_json('./train_data/' + t + '.json', story_dict)

preprocess_all_stories()   
save_json("triples_train.json", triples_dict)